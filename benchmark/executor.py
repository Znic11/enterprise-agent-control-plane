...
class BenchmarkExecutor:
    ...
    async def initialize(self):
        ...
        if "router_llm_config" in self.orchestrator_kwargs:
            ...
            self.orchestrator_kwargs["router_llm_client"] = self.router_llm_client
    ...
    def _initialize_router_llm(self, router_llm_config: "LLMConfig") -> None:
        """Initialize router LLM client with retry. Sets self.router_llm_client."""
        logger.info("Initializing router LLM...")
        self.router_llm_client = LLMClient(
            provider=router_llm_config.llm_provider,
            model=router_llm_config.llm_model,
            api_key=router_llm_config.llm_api_key,
            api_endpoint=router_llm_config.llm_api_endpoint,
            api_version=router_llm_config.llm_api_version,
            region=router_llm_config.llm_region,
            temperature=router_llm_config.temperature,
            max_tokens=router_llm_config.max_tokens,
            top_p=router_llm_config.top_p,
            effort=router_llm_config.effort,
            reasoning=router_llm_config.reasoning,
        )
        self.router_llm_client.llm = self.router_llm_client.llm.with_retry(
            retry_if_exception_type=(Exception,),
            wait_exponential_jitter=True,
            stop_after_attempt=3,
        )
        logger.info(f"✅ Router initialized: {router_llm_config.llm_provider}/{router_llm_config.llm_model}")
